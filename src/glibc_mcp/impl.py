...
def open(path: str, flags: int, mode: int | None = None) -> int:
    """
    The open() system call opens the file specified by path.  If the
    ...
    reference is unaffected if path is subsequently removed or
    ...
    O_DIRECTORY
           If path is not a directory, cause open() to fail.  This
    ...
    O_EXCL Ensure that this call creates the file: if this flag is
           ...
           followed: if path is a symbolic link, then open() fails
    ...
    O_NOCTTY
           If path refers to a terminal device—see tty(4)—it will not
    ...
    O_NOFOLLOW
           ...
           found while resolving components in the path prefix of the
           pathname.)
    ...
    O_PATH (since Linux 2.6.39)
           ...
           If path refers to an automount point that has not yet been
           triggered, so no other filesystem is mounted on it, then
           the call returns a file descriptor referring to the
...
def openat(dirfd: int, path: str, flags: int, mode: int | None = None) -> int:
    ...
    If the pathname given in path is relative, and dirfd is not a
