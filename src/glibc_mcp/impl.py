...
from .types import DIR_p, Dirent, dirent_p, Stat, stat as c_stat, Passwd, passwd_p
...
LIBC.getpwuid.restype = passwd_p
...
@mcp.tool()
def stat(path: str) -> tuple[Stat | None, int]:
    s = c_stat()
...
@mcp.tool()
def lstat(path: str) -> tuple[Stat | None, int]:
    s = c_stat()
...
@mcp.tool()
def getpwuid(uid: int) -> Passwd | None:
    p = LIBC.getpwuid(uid)
    if p:
        return Passwd(p.contents)
    else:
        return None
